...
import { ArrowRight, Code, Smartphone, BarChart3, Filter } from 'lucide-react';

const portfolioIcons = [Code, Smartphone, BarChart3, Filter];
...
export default function PortfolioHero() {
  ...
  const fullText = "Creative Portfolio";
  ...
  useEffect(() => {
    ...
    // Create floating portfolio icons
    const createFloatingIcons = () => {
      const icons: THREE.Mesh[] = [];
      
      portfolioIcons.forEach((IconComponent, index) => {
    ...
    <section ref={containerRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center" style={{ zIndex: 3 }}>
        ...
        >
          ...
          >
            {portfolioIcons.map((IconComponent, index) => (
              <motion.div
                key={index}
              ...
              >
                <div className="w-16 h-16 bg-gradient-to-br from-green-100 to-blue-100 dark:from-green-900/30 dark:to-blue-900/30 rounded-2xl flex items-center justify-center shadow-lg dark:shadow-dark-900/50 transition-colors duration-300">
                  <IconComponent size={32} className="text-green-600 dark:text-green-400" />
                </div>
                <span className="text-sm text-gray-600 dark:text-gray-300 font-medium transition-colors duration-300">Category {index + 1}</span>
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
